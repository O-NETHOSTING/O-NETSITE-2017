...
<?php
    ...
    $kveri = mysqli_query($con,"SELECT * FROM tiketi WHERE status='0'");
    ...
    if ($page == 1) {
		...
		$centar .= '<li class="active"><a href="/admin_support/&stranica=' . $page . '">' . $page . '</a></li>';
		$centar .= '<li><a href="/admin_support/&stranica=' . $add1 . '">' . $add1 . '</a></li> ';
	} else if ($page == $zadnja) {
		$centar .= '<a href="/admin_support/&stranica=' . $sub1 . '">' . $sub1 . '</a> ';
		$centar .= '<li><span style="color:#999999;">' . $page . '</span></li>';
	} else if ($page > 2 && $page < ($zadnja - 1)) {
		$centar .= '<a href="/admin_support/&stranica=' . $sub2 . '">' . $sub2 . '</a> ';
		$centar .= '<a href="/admin_support/&stranica=' . $sub1 . '">' . $sub1 . '</a> ';
		$centar .= '<span style="color:#999999;">' . $page . '</span>';
		$centar .= '<a href="/admin_support/&stranica=' . $add1 . '">' . $add1 . '</a> ';
		$centar .= '<a href="/admin_support/&stranica=' . $add2 . '">' . $add2 . '</a> ';
...
$kveri2 = mysqli_query($con,"SELECT * FROM tiketi WHERE status='0' ORDER BY id DESC LIMIT $prvi,$drugi");
	...
	$provera = mysqli_num_rows($con,"SELECT id FROM tiketi WHERE status='0'");
	...
	if($br_otvorenih_tiketa < 1) {
		...
		echo " <tr>
			<td>Trenutno nemate otvorenih tiketa</td>
